...
rule trinotate_hits:
    ...
    input:
        "results/trinotate/uniprot_sprot.pep",
        "results/trinotate/Pfam-A.hmm",
        fastafile = expand("{transcriptome}", transcriptome=config["reference"]["transcriptome"]),
        peptidefile = expand("{proteome}", proteome=config["reference"]["proteome"]),
        transdecoder_peptidefile = expand("{longest_orfs}", longest_orfs=config["reference"]["longest_orfs"]),
        geneTranscript_map = expand("{gene_to_transcript}", gene_to_transcript=config["reference"]["gene_to_transcript"])
    output:
